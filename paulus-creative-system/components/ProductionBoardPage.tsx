...
const ProductionBoardPage: React.FC<ProductionBoardPageProps> = ({ project, onUpdateProject, language, currentUser }) => {
  ...
  // Derive Storyboard Frames from Deck
  ...
  const handleAddItem = () => {
      const newItem: DirectionItem = {
          ...
          status: 'DRAFT',
          authorId: currentUser.id // Assign Author
      };
  ...
  const handleSyncToDeck = () => {
      // Create a new slide for this category
      const newSlide = {
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                <div className="flex items-center gap-2">
                    {/* Fixed typo: added missing opening bracket for h2 */}
                    <h2 className="font-bold text-gray-800 text-lg">{activeCategory}</h2>
                    <span className="text-xs text-gray-500 bg-white px-2 py-0.5 rounded border border-gray-200">{categoryItems.length} {t('Items', '개')}</span>
                </div>

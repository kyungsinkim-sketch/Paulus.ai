        ...
        <div className="max-w-7xl mx-auto h-full">
          ...
          {activeSection === 'FINANCIAL' && (
            <div className="space-y-8 animate-in fade-in">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-xl font-black text-gray-900 uppercase tracking-tight">Financial Overview</h2>
                  <p className="text-xs text-gray-400 mt-1">Salary distribution and payroll management.</p>
                </div>
                <button
                  onClick={() => exportToExcel(users)}
                  className="px-4 py-2 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-emerald-700 transition-all flex items-center gap-2"
                >
                  <FileText size={14} /> Export Payroll
                </button>
              </div>

              <div className="grid grid-cols-4 gap-6">
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                  <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Total Monthly Payroll</div>
                  <div className="text-3xl font-black text-gray-900">
                    ₩{users.reduce((acc, u) => acc + (u.monthlySalary || 0), 0).toLocaleString()}
                  </div>
                </div>
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                  <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Avg. Annual Salary</div>
                  <div className="text-3xl font-black text-blue-600">
                    ₩{Math.round(users.reduce((acc, u) => acc + (u.annualSalary || 0), 0) / users.length).toLocaleString()}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                <table className="w-full text-left">
                  <thead className="bg-gray-50 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">
                    <tr>
                      <th className="px-6 py-4">Employee</th>
                      <th className="px-6 py-4 text-center">Level / Class</th>
                  ...
                  <tbody className="divide-y divide-gray-50 text-sm">
                    {users.map(u => (
                      <tr key={u.id} className="hover:bg-gray-50">
                        <td className="px-6 py-3 font-bold text-gray-900">{u.name} <span className="text-gray-400 font-normal ml-1">({u.koreanName})</span></td>
                        <td className="px-6 py-3 text-center">
                          <span className="inline-block bg-gray-100 px-2 py-0.5 rounded text-[10px] font-bold text-gray-600">{u.level}</span>
                          <span className="inline-block ml-1 bg-blue-50 px-2 py-0.5 rounded text-[10px] font-bold text-blue-600">{u.salaryClass}</span>
                        </td>
                        <td className="px-6 py-3 text-center font-mono text-xs text-gray-500">{u.joinDate}</td>
                        <td className="px-6 py-3 text-right font-mono text-gray-600">₩{(u.monthlySalary || 0).toLocaleString()}</td>
                        <td className="px-6 py-3 text-right font-mono font-bold text-gray-900">₩{(u.annualSalary || 0).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {activeSection === 'PEOPLE' && (
            <div className="space-y-8 animate-in fade-in">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-xl font-black text-gray-900 uppercase tracking-tight">People & Performance</h2>
                  <p className="text-xs text-gray-400 mt-1">Detailed HR information and contact list.</p>
                </div>
              </div>
                ...
                <table className="w-full text-left">
                  <thead className="bg-gray-50 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100">
                    <tr>
                      ...
                      <th className="px-6 py-4">Dept / Pos</th>
                      <th className="px-6 py-4">Current Contact</th>
                    ...
                    {users.map(u => (
                      <tr key={u.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          ...
                          <div className="text-gray-400">{u.position} / {u.jobTitle}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="font-mono">{u.phoneNumber}</div>
                          <div className="text-blue-600">{u.email}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="font-bold">{u.university}</div>
                          <div className="text-gray-500">{u.major}</div>
                        </td>
                        <td className="px-6 py-4 text-gray-500 max-w-[200px] truncate" title={u.address}>
                          {u.address}
                        </td>
                      </tr>
                    ))}
